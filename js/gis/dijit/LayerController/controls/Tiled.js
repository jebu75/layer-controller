/*
 * arcgis tiled map service layer loader and control
 */
define([
    'dojo/_base/declare',
    'dojo/_base/lang',
    ...
    return declare([WidgetBase, TemplatedMixin, Contained], {
        ...
        postCreate: function() {
            //validate
            if (!this.layerParams) {
                console.log('TiledLayerControl error::layerParams option is required');
                this.destroy();
                return;
            }
            if (!this.map) {
                console.log('TiledLayerControl error::map option is required');
                this.destroy();
                return;
            }
        ...
        _initialize: function(layerParams, map) {
            ...
            //the layer
            this.layer = new ArcGISTiledMapServiceLayer((lp.secured) ? lp.url + '?token=' + lp.token : lp.url, {
                id: lp.id || null,
                visible: lp.visible,
                opacity: lp.opacity,
        ...
        _zoomToLayer: function() {
            ...
            if (layer.spatialReference === map.spatialReference) {
                ...
                if (esriConfig.defaults.geometryService) {
                    ...
                    console.log('TiledLayerControl _zoomToLayer::esriConfig.defaults.geometryService is not set');
                }
            }
        }
    });
